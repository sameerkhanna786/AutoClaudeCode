...
35. DONE: In `state.py`, add a `get_strategy_performance_report()` method that returns a formatted string showing success rate, average cost, and average duration per task type over the last 24 hours. Call this from the orchestrator/coordinator at startup to log which strategies are working best.
...
35. DONE: Added `get_strategy_performance_report()` method to `state.py:StateManager` that builds on existing `get_strategy_performance()` to return a formatted string showing success rate, average cost, and average duration per task type over the last 24 hours. Sorted by success rate descending. Called from both `orchestrator.py:run()` and `coordinator.py:run()` at startup via `logger.info()`. Added 3 tests in `tests/test_state.py` covering empty history, report formatting/ordering, and old record exclusion.
